...
%     end
% end
% sensoren1D = reshape(sensoren,nx*ny,1);
% p_index = find(sensoren1D == 1); 

% random positions
p_index = sort(randperm(nx*ny,64));
sensoren1D = zeros(nx*ny,1);
sensoren1D(p_index) = 1;
sensoren = reshape(sensoren1D,nx,ny);


m_rh = f(p_index,:);   
save('Messwerte_rh.mat','f','m_rh','p_index')

figure
for n = 1 : 5 : nt
    clf
    plot( f(:,n),'c-','LineWidth',1)
    hold on
    plot(p_index,m_rh(:,n),'r.','markersize',20)
    legend('Signal','Measure Points')
    xlim([0 nx*ny])
    ylim([-0.5 2.5])
