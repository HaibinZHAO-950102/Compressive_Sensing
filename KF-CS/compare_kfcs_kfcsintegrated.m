...
figure
for n = 1 : (size(f_e_wt_kf,2)-1)/50 : size(f_e_wt_kf,2)
    clf
    plot(x, f(n,:),'k-','LineWidth',3)
    hold on
    plot(x, f_e_wt_kf(:,n),'r-','LineWidth',3)
    hold on
    KFCSI = plot(x, f_e_wt_kf_integrated(:,n),'--','LineWidth',3)
    set(KFCSI,'Color',[0.1961 0.6314 0.5373])
    hold on
    legend('Origin','KF CS 6 Sensors','KF CS integrated 6 Sensors')
    xlim([0 10])
    ylim([-0.5 2.5])
    set(gca,'Fontsize',20)
    ...
    name = ['compare KFCS and KFCS integrated.gif'];
    if printfigure == 1
        if n==1
...
% error_wt_kf_integrated = f - f_e_wt_kf_integrated';
% 
% 
% zero = zeros(size(f));
% figure
...
similariy_wt_kf_integrated = zeros(size(f_e,2),1);
...
for i = 1 : size(f_e,2)
    ...
    similariy_wt_kf_integrated(i) = f(i,:) * f_e_wt_kf_integrated(:,i) / (norm(f(i,:)) * norm(f_e_wt_kf_integrated(:,i)));
end
...
error_wt_kf_integrated = 2 * (1-similariy_wt_kf_integrated);
...
hold on
plot(t(10:end),error_wt_kf(10:end),'m-','LineWidth',3)
hold on
plot(t(10:end),error_wt_kf_integrated(10:end),'Color',[0.1961 0.6314 0.5373],'LineWidth',3)
legend('KF-CS','KF 6','KF 11','CS WT KF 6','CS WT KF integrated 6')
