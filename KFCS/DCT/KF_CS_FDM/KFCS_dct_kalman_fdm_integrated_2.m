...
number = 12;
S = zeros(nt,number);
S(:,1:3) = [ones(nt,1)*20,ones(nt,1)*32,ones(nt,1)*45];
for t = 1 : nt
    k = 4;
    while k <= number
        temp = ceil(rand()*64);
...
for n = 1 : 0.5/Dt : nt
    ...
    hold on
    plot(x, f_e_integrated_2(:,n),'c-','LineWidth',3)
    hold on
    ...
    name = ['CS_dct_KF_FDM_integrated_1.gif'];
    if printfigure == 1
        if n==1
...
f_e_dct_kf_fdm_integrated_2 = f_e_integrated_2;
save('f_e_dct_kf_fdm_integrated_2.mat','f_e_dct_kf_fdm_integrated_2')
