...
f_e_dct_kf_integrated = zeros(nx,nt);
f_e_dct_kf_integrated(:,1) = f(:,1);

dz = zeros(64,nt);

T(:,1) = (Psi'*Psi)^-1*Psi'*f_e_dct_kf_integrated(:,1);
...
for t = 2 : nt
    ...
    f_e_dct_kf_integrated(:,t) = Psi * T(:,t);
    
    z(:, t) = DCT * Phi_KF * f_e_dct_kf_integrated(:,t);
end
...
for n = 1 : 0.5/Dt : nt
    ...
    hold on
    plot(x, f_e_dct_kf_integrated(:,n),'c-','LineWidth',3)
    hold on
    ...
    name = ['CS_dct_KF_modal_integrated_1.gif'];
    if printfigure == 1
        if n==1
...
f_e_dct_kf_modal_integrated_1 = f_e_dct_kf_integrated;
save('f_e_dct_kf_modal_integrated_1.mat','f_e_dct_kf_modal_integrated_1')
