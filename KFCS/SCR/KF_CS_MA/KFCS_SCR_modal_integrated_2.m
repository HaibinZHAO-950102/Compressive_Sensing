...
f_e_mt_kf_integrated_2 = zeros(nx,nt);
f_e_mt_kf_integrated_2(:,1) = f(:,1);

dz = zeros(64,nt);

T(:,1) = (Psi'*Psi)^-1*Psi'*f_e_mt_kf_integrated_2(:,1);
...
for i = 1 : nt
    f_e_mt_kf_integrated_2(:,i) = Psi * T(:,i);
end
...
for n = 1 : 0.5/Dt : nt
    ...
    hold on
    plot(x, f_e_mt_kf_integrated_2(:,n),'c-','LineWidth',3)
    hold on
    ...
    name = ['CS_mt_KF_modal_integrated_2.gif'];
    if printfigure == 1
        if n==1
...
% f_e_mt_kf_modal_integrated_2 = f_e_mt_kf_integrated_2;
% save('f_e_mt_kf_modal_integrated_2.mat','f_e_mt_kf_modal_integrated_2')
