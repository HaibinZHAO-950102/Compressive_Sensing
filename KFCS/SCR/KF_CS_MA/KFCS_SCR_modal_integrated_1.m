...
Ce = zeros(G+1, G+1, nt);
Ce(:,:,1) = eye(G+1) * 10 ^ 10;
Cv = eye(length(p_index)) * 0.03;  % Messunsicherheit
Cw = eye(G+1) * 0.5;  % Systemrauschen

f_e_mt_kf_integrated = zeros(nx,nt);
f_e_mt_kf_integrated(:,1) = f(:,1);

dz = zeros(64,nt);

T(:,1) = (Psi'*Psi)^-1*Psi'*f_e_mt_kf_integrated(:,1);
...
for t = 2 : nt
    ...
    f_e_mt_kf_integrated(:,t) = Psi * T(:,t);
    
    z(:, t) = MT * Phi_KF * f_e_mt_kf_integrated(:,t);
end
...
for n = 1 : 0.5/Dt : nt
    ...
    hold on
    plot(x, f_e_mt_kf_integrated(:,n),'c-','LineWidth',3)
    hold on
    ...
    name = ['CS_mt_KF_modal_integrated_1.gif'];
    if printfigure == 1
        if n==1
...
f_e_mt_kf_modal_integrated_1 = f_e_mt_kf_integrated;
save('f_e_mt_kf_modal_integrated_1.mat','f_e_mt_kf_modal_integrated_1')
