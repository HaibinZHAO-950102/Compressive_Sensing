...
for kkk = 1 : 10
    for iii = 2 : 11
        for jjj = 2 : 11
            ...
            f_e(:,1) = f_mu(:,1);
            Ce = speye(nx) * 1;
            Cv = speye(length(S)) * sigma_mu;  % Messunsicherheit
            Cw = phi * eye(51)* phi' * sigma_sr;
            for t = 2 : nt
...
%                 name = ['Kalman_fdm_',num2str(M),'_rh.gif'];
%                 if printfigure == 1
%                     if n==1
...
%             end
            ...
            name = ['f_e_',num2str(kkk),'_',num2str(iii),'_',num2str(jjj)];
            save(name,'f_e')
        end
    end
end
