...
setmesh('Initial Condition','$x$','$y$','$T$','T_2D_homo_modal_inital_condition_1',printfigure)
...
setplt('Eigenfunctions $\varphi$','$x$','$value$','T_2D_homo_modal_Eigenfunctions_phi_1',printfigure)
...
set(TEXT,'Interpreter','latex')
setplt('Eigenfunctions $\psi$','$y$','$value$','T_2D_homo_modal_Eigenfunctions_psi_1',printfigure)
...
for n = 1 : 4 : N_time
    ...
    zlim([-1.5 1.5])
    pbaspect([1 Length_y/Length_x 0.5])
    setmesh('Temperature Distribution','$x$','$y$','$T$','T_2D_homo_modal_1',0)
    ...
    if printfigure == 1
        if n==1
             imwrite(imind,cm,'T_2D_homo_modal_1.gif','gif', 'Loopcount',inf,'DelayTime',1e-6);
        else
             imwrite(imind,cm,'T_2D_homo_modal_1.gif','gif','WriteMode','append','DelayTime',1e-6);
        end
    end
end
